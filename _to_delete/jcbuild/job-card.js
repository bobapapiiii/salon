...
import { fmtTime } from './booking-types.js';
...
function groupInfo(a, dayAppts, clients) {
    if (!a.parallelGroup)
        return undefined;
 ...
 */
export function buildJobCard(group, ctx) {
    const sorted = [...group].sort((x, y) => x.startMin - y.startMin);
    const first = sorted[0];
    const techName = (id) => ctx.techs.find((t) => t.id === id)?.name ?? '';
    const client = ctx.clients.find((c) => c.name === first.clientName || c.id === first.guestOf);
    ...
    const services = sorted.map((a) => {
        ...
        return {
            ...
            preferLabel: !requested && a.requestedTechChoice === 'pref-female' ? 'F PREFER'
    ...
    const notes = [...new Set(sorted.map((a) => a.notes?.trim()).filter((n) => !!n))];
    return {
...
export function cardGroupsFor(appts) {
    ...
    return [...byPerson.values()].sort((x, y) => Math.min(...x.map((a) => a.startMin)) - Math.min(...y.map((a) => a.startMin)));
}
// ── rendering ──────────────────────────────────────────────────────────────
const esc = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
const mins = (n) => `${n}m`;
/** blank ruled rows the tech writes the real service, color and price into */
function writeIn(width, rows) {
    if (width === 80) {
...
function cardHtml(c, width, salonName) {
...
export function jobCardsHtml(cards, width, salonName) {
    return `<!doctype html>
 ...
 */
export function printJobCards(cards, width, salonName) {
    if (cards.length === 0)
        return true;
